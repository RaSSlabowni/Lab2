import math
try:
    number = int(input("Введите целое число от 1 до 9999: ")) # Ввод значений
...
except ValueError: # Проверка на правильность значений
    print('Введены некорректные значения. Попробуйте снова')
    input('Нажмите ENTER чтобы продолжить')
else:    
    if number <= 9: #Проверка на значения
        print(number_list[number].capitalize(), end=" ") #Вывод значений в виде слов
        ...
        if tens == 0:
            ...
            rint(decades_list[twos].capitalize() + " " + number_list[tens], end=" ")
